...
## Контекст використання (Intune / Autopilot)

Кастомний ISO зручно використовувати як **перший етап** розгортання у поєднанні з Microsoft Intune / Autopilot:

1. **Інсталяція з кастомного ISO** — Windows ставиться разом із попередньо інтегрованими програмами та драйверами.
2. **OOBE + `SetupComplete.cmd`** — після першого входу під SYSTEM виконуються MSI/EXE-інсталяції, які залежать від OOBE-фази (наприклад, `ALLUSERS=1`).
3. **Реєстрація в Autopilot/Intune** — оператор знімає hardware hash (`Get-WindowsAutopilotInfo`) і завантажує у Microsoft Intune.
4. **Передача керування Intune** — далі вся політика, оновлення та моніторинг ідуть через MDM.
